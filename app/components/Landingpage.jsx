    ...
    <div ref={sectionRef} >
      <div  className="w-full h-auto">
        <div className="text-structure px-2 sm:px-10 lg:px-20 mt-[10vh]">
          {["WE DELIVER", "INNOVATIVE", "TECHNOLOGY"].map((item, index) => (
            <div key={index} className="masker">
              <div className="w-fit leading-[3rem] sm:leading-[4rem] md:leading-[4.8rem] lg:leading-[7.2rem] flex items-center">
                {index === 1 && (
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: "10vw" }}
                    transition={{ ease: [0.45, 0, 0.55, 1], duration: 1 }}
                    className="w-[19vw] sm:w-[16vw] lg:w-[10vw] h-[2.3rem] sm:h-[3.2rem] md:h-[4rem] lg:h-[5.8rem] bg-[url('/herosm.svg')] bg-cover bg-center rounded-[0.2rem] lg:rounded-[1rem]"
                  ></motion.div>
                )}
                <h1 className="main-title-landing uppercase text-[3rem] sm:text-[4.5rem] md:text-[5.5rem] lg:text-[8rem] tracking-tight font-semibold">
                  {item}
                </h1>
              </div>
            </div>
